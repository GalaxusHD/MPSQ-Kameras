...
public final class BildschirmErstellenScreen extends Screen {
    ...
    @Override protected void init() {
        ...
        modeButton = addDrawableChild(CyclingButtonWidget.builder(LocalScreenStore.ScreenInputType::text).values(LocalScreenStore.ScreenInputType.values()).initially(selectedMode)
                .build(cx - fieldWidth / 2, y, fieldWidth, 20, Text.translatable("gui.mpsqcamera.erstellen.modus"), (b, mode) -> { selectedMode = mode; updateMode(mode); })); y += 28;
    ...
    private void openCameraPicker() { client.setScreen(new CameraPickerScreen(this, camera -> { selectedCameraId = camera.id(); selectedCameraName = camera.name(); selectedMode = LocalScreenStore.ScreenInputType.CAMERA; status = ""; })); }
